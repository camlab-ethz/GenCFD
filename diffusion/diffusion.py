...
def sigma2logsnr(
  ...
  inverse = lambda logsnr: sigma.inverse(th.exp(th.as_tensor(-logsnr / 2), device=device))
...
def logsnr2sigma(
  ...
  inverse = lambda sigma: logsnr.inverse(-2 * th.log(th.as_tensor(sigma, device=device)))
  return InvertibleSchedule(forward, inverse)
...
class Diffusion:
  ...
  def create_variance_preserving(
    ...
    scale = lambda t: th.sqrt(var / (var + th.square(th.as_tensor(sigma(t), device=device))))
  ...
  def create_variance_exploding(
    ...
    scaled_inverse = lambda y: th.as_tensor(sigma.inverse(y / data_std), device=device)
    ...
    return cls(
      scale=lambda s: th.as_tensor(s/s, device=device), 
      sigma=scaled_sigma
    )
...
def tangent_noise_schedule(
  ...
  inverse = lambda y: in_rescale.inverse(th.arctan(th.as_tensor(out_rescale.inverse(y), device=device))) 
...
def exponential_noise_schedule(
  ...
  sigma = lambda t: out_rescale(th.pow(th.as_tensor(base, device=device), in_rescale(t)))
  inverse = lambda y: in_rescale.inverse(  # pylint:disable=g-long-lambda
...
def edm_weighting(data_std: float = 1.0, device: th.device = None) -> NoiseLossWeighting:
  ...
  def _weight_fn(sigma: Tensor) -> Tensor:
    return (th.square(th.tensor(data_std, device=device)) + th.square(sigma)) / th.square(
        data_std * sigma
    )
